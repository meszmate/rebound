...
(function () {
  ...
  function genScript(name, c) {
    var lines = [
      ...
      "  // A stepped (HOLD-out) segment has no interpolation to ease; skip it.",
      "  function holdSeg(p, i) { try { return p.keyOutInterpolationType(i) === KeyframeInterpolationType.HOLD; } catch (e) { return false; } }",
      ...
      "      }",
      "      // Convert ONLY the side being written, preserving a HOLD (or any",
      "      // other interpolation) on the side this segment does not touch.",
      "      p.setInterpolationTypeAtKey(a, p.keyInInterpolationType(a), KeyframeInterpolationType.BEZIER);",
      "      p.setTemporalEaseAtKey(a, p.keyInTemporalEase(a), outArr);",
      "      p.setInterpolationTypeAtKey(b, KeyframeInterpolationType.BEZIER, p.keyOutInterpolationType(b));",
