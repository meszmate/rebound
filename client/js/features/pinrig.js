...
;(function (R) {
  ...
  function overlaySvg(st, h) {
    ...
    if (st.pins) for (var p = 0; p < verts.length; p++) kids.push(svg('circle', { cx: r1(verts[p][0]), cy: r1(verts[p][1]), r: mr, fill: ac, stroke: 'var(--rb-bg)', 'stroke-width': sw * 0.6 }));
  ...
  function mount(ctx) {
    ...
    function applyState(s) {
      ...
      pickers[0].set(st.accent); pickers[1].set(st.label);
